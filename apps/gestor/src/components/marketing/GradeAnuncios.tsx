...
import { AlertTriangle, Check, Loader2, Lock, Plug, RefreshCw, X } from 'lucide-react'
...
const thStyle = (largura?: number): React.CSSProperties => ({
...
export function GradeAnuncios() {
  ...
  const carregarPortais = useCallback(async () => {
    try {
      setPortais(await api.get<Portal[]>('/v1/anuncios/portais'))
    } catch {
  ...
  const carregarGrade = useCallback(async (silencioso = false) => {
    ...
    try {
      ...
      const [g, p] = await Promise.all([
        api.get<Grade>('/v1/anuncios/grade', params),
        api.get<Pendencia[]>('/v1/anuncios/pendencias'),
      ])
      setGrade(g)
      setPendencias(p)
    } catch {
  ...
  const confirmarBaixa = useCallback(async (anuncioId: string, rotulo: string) => {
    setOcupado(true)
    try {
      await api.post(`/v1/anuncios/${anuncioId}/confirmar-baixa`)
  ...
  async function sincronizarTodos() {
    setOcupado(true)
    try {
      const r = await api.post<{ enfileirados: number }>('/v1/anuncios/sincronizar-todos')
      showToast(
        r.enfileirados
        ...
        disponiveis.map(async (p) => {
          try {
            const r = await api.post<{ sucesso: boolean; erro?: string }>(
              `/v1/anuncios/portais/${p.nome}/testar`,
            )
  ...
  return (
    <div>
      ...
      <div className="glass-card" style={{ padding: 0, overflow: 'hidden' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 900 }}>
            <thead>
              <tr>
                ...
                {portais.map((p) => (
                  <th key={p.nome} style={{ ...thStyle(104), textAlign: 'center' }}>{p.rotulo}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              ...
              {!carregando && grade.itens.map((linha) => (
                ...
                >
                  ...
                  <td style={{ ...tdStyle, fontVariantNumeric: 'tabular-nums', fontSize: 13, whiteSpace: 'nowrap' }}>
                  ...
                  {portais.map((portal) => {
                    ...
                    return (
                      <td key={portal.nome} style={{ ...tdStyle, textAlign: 'center' }}>
                        <ChipCelula
                          celula={celula}
                          portal={portal}
        ...
        <div style={{
          padding: '11px 14px', fontSize: 12, color: 'var(--sv-text-muted)',
          borderTop: '1px solid var(--sv-border)', display: 'flex',
          justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap',
        }}>
          <span>{grade.itens.length} de {grade.total} veículo(s)</span>
          {totalPaginas > 1 && (
            <span style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              <button
                className="btn btn-outline" style={{ fontSize: 12, padding: '4px 10px' }}
                disabled={pagina + 1 >= totalPaginas} onClick={() => setPagina((p) => p + 1)}
              >Próxima</button>
            </span>
          )}
        </div>
