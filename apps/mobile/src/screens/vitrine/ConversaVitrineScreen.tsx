...
export default function ConversaVitrineScreen({ route }: VitrineScreenProps<'ConversaVitrine'>) {
  ...
  const enviarMut = useMutation({
    mutationFn: (t: string) => vitrineService.enviar(id, t),
    onMutate: async (t) => {
      const chave = ['vitrine', 'mensagens', id]
      await queryClient.cancelQueries({ queryKey: chave })
      const anteriores = queryClient.getQueryData<Mensagem[]>(chave)
      const otimista: Mensagem = { id: `tmp-${Date.now()}`, conversa_id: id, autor: 'cliente', texto: t, created_at: new Date().toISOString(), lida: true }
      queryClient.setQueryData<Mensagem[]>(chave, (v) => [...(v ?? []), otimista])
      return { anteriores }
    },
    onError: (_e, _t, ctx) => {
      if (ctx?.anteriores) queryClient.setQueryData(['vitrine', 'mensagens', id], ctx.anteriores)
    },
    onSettled: () => queryClient.invalidateQueries({ queryKey: ['vitrine'] }),
  })
    ...
    <View
      style={[
        ...
        minha
          ...
          : { alignSelf: 'flex-start', backgroundColor: dark ? colors.surfaceElevated : '#ffffff', borderBottomLeftRadius: 4, borderWidth: dark ? 0 : 1, borderColor: colors.border },
      ]}
    >
      <Txt style={{ fontFamily: fonts.regular, fontSize: 15, lineHeight: 20, color: minha ? colors.onPrimary : colors.text }}>{mensagem.texto}</Txt>
      <Txt style={{ fontFamily: fonts.regular, fontSize: 10, color: minha ? 'rgba(255,255,255,0.7)' : colors.textMuted, alignSelf: 'flex-end', marginTop: 2 }}>
