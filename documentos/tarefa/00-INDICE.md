...
> ⚠️ **ANTES de mexer em banco, migrations ou deploy:** leia [ARMADILHAS-PRODUCAO.md](ARMADILHAS-PRODUCAO.md). Quatro coisas já quebraram produção (datetime aware em coluna naive, migrations SQLite-only que não rodam em Postgres, deploy Vercel fora da raiz do monorepo, storage sem os secrets S3 caindo no disco efêmero do Fly) e a mensagem de erro de cada uma aponta para a causa errada.
...
> 📐 **Specs de design/implementação:** não ficam como arquivos soltos — o essencial de cada feature (decisões + o que foi entregue) é condensado direto em `MELHORIAS.md`/`TAREFAS.md` assim que fecha. **Regra:** documentação de trabalho vai para `documentos/tarefa/`; planos estratégicos em `documentos/planos/`; registros encerrados em `documentos/historico/`. Índice mestre: [documentos/README.md](../README.md). Não criar `docs/` nem pastas soltas na raiz.
